...
class OpenSSLServerDomainConfig(DomainConfig):
    def __init__(self, test_id, domain, expected_warnings=None, expected_failures=None, manual_cipher_checks=False):
        self._manual_cipher_checks = manual_cipher_checks
        super().__init__(test_id, domain, expected_warnings=expected_warnings, expected_failures=expected_failures)
...
class PreTLS13DomainConfig(DomainConfig):
    def __init__(self, test_id, domain, expected_warnings=None, expected_failures=None, expected_info=None, lang="en"):
        self._lang = lang
        super().__init__(
            test_id,
            domain,
...
ncsc_20_tests = [
    ...
    PreTLS12DomainConfig(
        ...
        expected_failures={TESTS.TLS_CIPHER_ORDER},
    ),
    PreTLS12DomainConfig(
        "NCSC20" "-Table12:TLS10",
    ...
    PreTLS13DomainConfig("NCSC20" "-Table1:TLS12" "-Table10:FFDHE4096", "tls12only.test.nlnetlabs.tk"),
    GoodDomain(
        "NCSC20"
        "-Table1:TLS1213"
        "-Table2:RSAEXPPSK"
    ...
    PreTLS13DomainConfig(
        "NCSC20" "-Table1:TLS12" "-Table6:LegacyBadCiphers",
    ...
    PreTLS13DomainConfig(
        "NCSC20" "-Table1:TLS12" "-Table6:LegacyPhaseOutCiphers",
    ...
    PreTLS13DomainConfig(
        "NCSC20" "-Table1:TLS12" "-Table6:ModernBadCiphers",
    ...
    PreTLS13DomainConfig(
        "NCSC20" "-Table1:TLS12" "-Table6:ModernPhaseOutCiphers",
        ...
        expected_info={TESTS.TLS_HASH_FUNC: [[MustMatch("not applicable")]]},
    ),
    ...
    PreTLS13DomainConfig(
        "NCSC20" "-Table1:TLS12" "-Table10:FFDHE2048",
    ...
    PreTLS13DomainConfig("NCSC20" "-Table1:TLS12" "-Table10:FFDHE3072", "tls12onlyffdhe3072.test.nlnetlabs.tk"),
    # This domain doesn't use an NCSC 2.0 approved DH finite-field group.
    PreTLS13DomainConfig(
        "NCSC20" "-Table1:TLS12" "-Table10:OtherGroups",
    ...
    PreTLS13DomainConfig(
        "NCSC20" "-Table1:TLS12" "-BUG:LongGenerator",
    ...
    DomainConfig(
        "NCSC20" "-Table1:TLS12" "-Table1:TLS13" "-Table5:No",
    ...
    DomainConfig(
        "NCSC20" "-Table1:TLS12" "-Table1:TLS13" "-Table6:ModernPhaseOutCiphers",
    ...
    PreTLS13DomainConfig(
        "NCSC20" "-Table11:TLS12" "-Table13:TLS12",
        "tls1213tlscompression.test.nlnetlabs.tk",
        expected_failures={TESTS.TLS_COMPRESSION},
    ),
    GoodDomain("NCSC20" "-Table1:TLS13" "-Table14:Off", "tls13only.test.nlnetlabs.tk"),
    # This website virtual host configuration deliberately does not do OCSP
    # stapling.
    DomainConfig(
        "NCSC20" "-Table1:TLS1213" "-Table15:Off",
    ...
    BadDomain("NCSC20" "-Table1:TLS13" "-Table14:On", "tls130rtt.test.nlnetlabs.tk", {TESTS.TLS_ZERO_RTT}),
    ...
    DomainConfig(
        "NCSC20" "-Table1:TLS13" "-Table15:OnInvalid",
    ...
    GoodDomain("NCSC20-GuidelineB2-5:TLS13", "tls13onlyhonorclientcipherorder.test.nlnetlabs.tk"),
]

other_tests = [
    # This website virtual host configuration deliberately fails to serve a
    # HSTS response header
    DomainConfig("HSTS:NONE", "tls1213nohsts.test.nlnetlabs.tk", expected_failures={TESTS.HTTPS_HTTP_HSTS}),
    ...
    DomainConfig(
        ...
        expected_failures={TESTS.IPV6_WEB_ADDRESS},
        expected_not_tested={
            TESTS.IPV6_WEB_REACHABILITY,
            TESTS.IPV6_WEB_SAME_WEBSITE,
        },
    ...
    # than on IPv6
    DomainConfig(
        "IPV6:DIFFTOIPV4", "tls13onlydiffipv4ipv6.test.nlnetlabs.tk", expected_failures={TESTS.IPV6_WEB_SAME_WEBSITE}
    ),
]
...
mail_tests = [
    PostfixTLS12Config("mail test", "tls12only.test.nlnetlabs.tk", expected_info={TESTS.TLS_CLIENT_RENEG}),
    PostfixTLS12Config("mail test", "tls1213prioritizechacha.test.nlnetlabs.tk"),
    DomainConfig(
        "mail test",
        "tls12onlynoip.test.nlnetlabs.tk",
        expected_error={
    ...
    PostfixTLS13Config("mail", "tls13ipv4only.test.nlnetlabs.tk", expected_failures={TESTS.IPV6_MAIL_REACHABILITY}),
    PostfixTLS13Config(
        "mail",
        "tls13ipv6onlynoipv4.test.nlnetlabs.tk",
    ),
    PostfixTLS13Config(
        "mail",
        "tls13ipv4onlynoipv6.test.nlnetlabs.tk",
        ...
        expected_not_tested={TESTS.IPV6_MAIL_REACHABILITY},
    ),
    PostfixTLS13Config("mail test", "tls13only.test.nlnetlabs.tk"),
    PostfixTLS13Config("mail test", "tls130rtt.test.nlnetlabs.tk"),
]
            ...
            for i, row in enumerate(UX.get_table_values(selenium, test_title)):
                ...
                if i < len(expected_details_table):
                    ...
                    assert regex_mode is True, (
                        f"At row {i} in result table of test '{test_title}': "
                        f"table has more rows than expected, either use a regex to match all "
                        f"rows or define more expected rows"
                    )
                ...
                for j, col in enumerate(row):
                    ...
                    if isinstance(expected_col, DetailTableCellMatcher):
                        ...
                        if isinstance(expected_col, MustMatch):
                            assert matched, (
                                f"At row {i} col {j} in result table of test '{test_title}': cell "
                                f"value '{col}' does not match specified regex '{expected_col}'"
                            )
                            ...
                            assert False, (
                                f"At row {i} col {j} in result table of test '{test_title}': expected "
                                f"value '{type(expected_col)}' must be one of type string, MustContain "
                                f"or MustMatch"
                            )
                    else:
...
@pytest.mark.parametrize("domain_config", other_tests, ids=domainconfig_id_generator)
def test_others(selenium, domain_config):
    assess_website(selenium, domain_config)


@pytest.mark.parametrize("domain_config", nl_translation_tests, ids=domainconfig_id_generator)
...
@pytest.mark.parametrize("iana_cipher", ncsc_20_good_ciphers, ids=iana_cipher_id_generator)
def test_ncsc_good_ciphers(selenium, iana_cipher):
    assess_website(
        selenium, PreTLS13DomainConfig("ncsc_good_ciphers", iana_cipher_to_target_server_fqdn("GOOD", iana_cipher))
    )


@pytest.mark.parametrize("iana_cipher", ncsc_20_sufficient_ciphers, ids=iana_cipher_id_generator)
def test_ncsc_sufficient_ciphers(selenium, iana_cipher):
    assess_website(
        selenium,
        PreTLS13DomainConfig("ncsc_sufficient_ciphers", iana_cipher_to_target_server_fqdn("SUFFICIENT", iana_cipher)),
    )


@pytest.mark.parametrize("iana_cipher", ncsc_20_phaseout_ciphers, ids=iana_cipher_id_generator)
def test_ncsc_phaseout_ciphers(selenium, iana_cipher):
    ...
    domain_config = PreTLS13DomainConfig(
        "ncsc_phaseout_ciphers",
        iana_cipher_to_target_server_fqdn("PHASEOUT", iana_cipher),
        expected_warnings=expected_warnings,
    )

    if ssh2_hash_function_supported is False:
        domain_config.expected_warnings[TESTS.TLS_HASH_FUNC] = [[MustMatch("no")]]
    elif ssh2_hash_function_supported is None:
        domain_config.expected_info[TESTS.TLS_HASH_FUNC] = [[MustMatch("not applicable")]]

    assess_website(selenium, domain_config)


@pytest.mark.parametrize("domain_config", mail_tests, ids=domainconfig_id_generator)
...
def test_batch_web_domains(request, selenium, batch_base_urls, batch_domain):
    assess_website(selenium, GoodDomain("batch_domain", batch_domain), base_urls=batch_base_urls, request=request)


# 'parametrized' by conftest.py::pytest_generate_tests()
def test_batch_mail_domains(request, selenium, batch_base_urls, batch_domain):
    assess_mail_servers(selenium, GoodDomain("batch_domain", batch_domain), base_urls=batch_base_urls, request=request)
