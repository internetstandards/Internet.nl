...
macro_expand = (
    (
        CaselessLiteral('%{') + macro_letter + Optional(transformers)
        + ZeroOrMore(delimiter) + CaselessLiteral('}'))
    | CaselessLiteral('%%') | CaselessLiteral('%_')
    | CaselessLiteral('%-'))
...
toplabel = (
    (
        Optional(Word(nums)) + Word(alphas, exact=1)
        + Optional(Word(alphanums)))
    | (
        Word(alphanums) + CaselessLiteral('-')
        + Optional(Word(alphanums + '-')))
...
ip6_cidr_length = (
    CaselessLiteral('/') + Regex('(12[0-8]|1[01][0-9]|[1-9][0-9]|[0-9])'))
ip4_cidr_length = CaselessLiteral('/') + Regex('(3[0-2]|[12][0-9]|[0-9])')
dual_cidr_length = (
    Optional(ip4_cidr_length) + Optional(
        CaselessLiteral('/') + ip6_cidr_length))
...
exists = Combine(
    Optional(qualifier) + CaselessLiteral('exists:') + domain_spec)
ip6 = Combine(
    Optional(qualifier) + CaselessLiteral('ip6:')
    + Regex('[^ ]*').setParseAction(_parse_ipv6))
ip4 = Combine(
    Optional(qualifier) + CaselessLiteral('ip4:') + ip4_network
    + Optional(ip4_cidr_length))
ptr = Combine(
    Optional(qualifier) + CaselessLiteral('ptr')
    + Optional(CaselessLiteral(':') + domain_spec))
mx = Combine(
    Optional(qualifier) + CaselessLiteral('mx')
    + Optional(CaselessLiteral(':') + domain_spec)
    + Optional(dual_cidr_length))
a = Combine(
    Optional(qualifier) + CaselessLiteral('a')
    + Optional(CaselessLiteral(':') + domain_spec)
    + Optional(dual_cidr_length))
include = Combine(
    Optional(qualifier) + CaselessLiteral('include:') + domain_spec)
...
record = (
    version + Group(terms).setResultsName('terms')
    + ZeroOrMore(SP) + StringEnd())
