...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument('server_or_url', help='Specifying a URL will output the page body and set the exit code to the HTTP response code. Specifing an FQDN will connect and print openssl s_client -connect like output.')
        parser.add_argument('--port', type=int, default=443)
        parser.add_argument('--conn', choices=['auto', 'debug', 'modern'], default='auto')
        parser.add_argument('--tls-version', choices=['auto', 'SSLv2', 'SSLv3', 'TLSv1.0', 'TLSv1.1', 'TLSv1.2', 'TLSv1.3'], default='auto')
    ...
    def handle(self, *args, **options):
        ...
        with conn_class(**kwargs) as conn:
            ...
            self.stdout.write(f'New, {conn.get_ssl_version().name}, Cipher is {conn.get_current_cipher_name()}')
            ...
            if options['calc_score']:
                ...
                self.stdout.write(f'Score Header   : {CipherScoreAndSecLevel.get_score_header()}')
                self.stdout.write(f'Score Bit Flags: {CipherScoreAndSecLevel.format_score(CipherScoreAndSecLevel.calc_cipher_score(ci))}')
