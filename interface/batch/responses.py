...
def unknown_request_response():
    return api_response(
        {"error": {"label": "unknown-request", "msg": "This request_id does not exist for the user."}}, status_code=404
    )
...
def bad_client_request_response(text):
    return api_response({"error": {"label": "bad-request", "msg": text}}, status_code=400)


def general_server_error_response(text="General server error. Please report this if it keeps happening"):
    return api_response({"error": {"label": "server-error", "msg": text}}, status_code=500)
