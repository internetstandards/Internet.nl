...
class Command(BaseCommand):
    ...
    def output_cli(self, headers, users):
        ...
        for s in res:
            lines = ["{:<{}}".format(x, max_length[i]) for i, x in enumerate(s)]
    ...
    def output_json(self, headers, users):
        res = []
        for user in users:
            res.append({headers[i].lower(): x for i, x in enumerate(user)})
        self.info(json.dumps(res))

    def list_users(self):
        users = [
            (f"{u.id}", u.username, u.name, u.organization, u.email) for u in BatchUser.objects.order_by("id").all()
        ]
        if not users:
    ...
    def register_user(self):
        try:
            ...
            raise CommandError(f"The user '{self.username}' already exists. Try " f"`api_users update` instead.")
        except BatchUser.DoesNotExist:
            create_batch_user(self.username, self.name, self.organization, self.email)
    ...
    def update_user(self):
        try:
            ...
            raise CommandError(f"The user '{self.username}' does not exist. Try " f"`api_users register` instead.")
    ...
    def remove_user(self):
        try:
            ...
            raise CommandError(f"The user '{self.username}' does not exist.")

    def add_arguments(self, parser):
        subparsers = parser.add_subparsers(
            title="sub-commands", description="Available sub-commands", dest="sub-command", required=True
        )

        list_parser = subparsers.add_parser("list", help="List the registered API users")
        list_parser.add_argument(
            "-f",
            "--format",
            default=FormatType.cli.name,
            choices=[FormatType.cli.name, FormatType.csv.name, FormatType.json.name],
        )
        list_parser.set_defaults(func=self.list_users)

        register_parser = subparsers.add_parser("register", help="Register a new API user")
        ...
        update_parser = subparsers.add_parser("update", help="Update an existing API user")
        ...
        remove_parser = subparsers.add_parser("remove", help="Remove an existing API user")
        remove_parser.set_defaults(func=self.remove_user)
        remove_parser.add_argument("-u", "--username", required=True)
        remove_parser.add_argument("-y", "--yes", action="store_true", help="Assume yes to removal prompt")
