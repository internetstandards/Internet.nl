...
class Command(BaseCommand):
    help = "Removes a domain from the Hall of Fame by removing the relevant 100% " "reports for that domain."
    ...
    def remove_domain(self, domain):
        for model in DomainTestReport, MailTestReport:
            try:
                latest_non_100_report = model.objects.filter(domain=domain, score__lt=100).order_by("-id")[0]
                id = latest_non_100_report.id
            except IndexError:
                id = 0

            res = model.objects.filter(domain=domain, id__gt=id, score=100).delete()
