...
class Command(BaseCommand):
    help = "Generate the batch API documentation also based on configured " "values."
    ...
    def replace_text(self, api_doc):
        ...
        for name, path in REPLACE_MAP.items():
            ...
            current_node[path[-1]] = current_text.replace(f"@@{name}@@", new_text, 1)
    ...
    def add_custom_results(self, api_doc):
        ...
        for custom in (CUSTOM_RESULTS_MAP[r] for r, active in settings.BATCH_API_CUSTOM_RESULTS.items() if active):
            if custom.required:
